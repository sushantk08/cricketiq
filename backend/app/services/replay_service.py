...
def get_match_decision_points(
    ...
    """Extract tactical turning moments in Innings 2 as replay candidates."""
    inn2 = (
        db.query(Innings)
    ...
    # Select beginning of critical overs (e.g. death overs 15-19 and middle phase 10-14)
    critical_overs = (
        db.query(Delivery)
        .filter(
    ...
    for d in critical_overs:
        ...
        reason = f"{phase} phase bowling decision with {runs_required} runs required off {balls_remaining} balls."
...
def simulate_decision_replay(
    ...
    inn2 = (
        db.query(Innings)
        .filter(
            Innings.match_id == req.match_id, Innings.innings_number == 2
        )
        .first()
    )
    inn1 = (
        db.query(Innings)
        .filter(
            Innings.match_id == req.match_id, Innings.innings_number == 1
        )
        .first()
    )
    if not inn2 or not inn1:
    ...
    # State before the over started
    runs_before = first_ball.cumulative_runs - first_ball.runs_batter
    wickets_before = (
        first_ball.cumulative_wickets - (1 if first_ball.is_wicket else 0)
    )
    ...
    # Baseline expectations from bowler's phase economy or overall economy
    if (
        alt_stats
    ...
    # Alternative state after simulated over
    ...
    # Decision Impact on defending (bowling) team's win probability:
    # Higher alternative win prob for defending team means alternative was better
    ...
    # Formula: Clamped around 50 adjusted by impact magnitude
    # If actual decision yielded better bowling probability, score > 50; if alternative yielded better, score < 50
    quality_score = float(np.clip(50.0 - (impact * 1.5), 5.0, 95.0))

    if impact > 5.0:
        verdict = (
            f"Suboptimal Choice: {alt_bowler.name} was statistically favored"
        )
        explanation = (
            f"Bowling {alt_bowler.name} in over {req.over_number} had an"
            f" expected economy of {exp_runs_alt:.1f} rpo versus"
            f" {actual_runs_scored} runs conceded by"
            f" {first_ball.bowler.name}. Estimated defensive win probability"
            f" would have improved by {impact:+.1f} percentage points."
        )
    elif impact < -5.0:
        verdict = (
            f"Sound Decision: {first_ball.bowler.name} was the superior"
            " option"
        )
        explanation = (
            f"{first_ball.bowler.name} delivered an effective over"
            f" ({actual_runs_scored} runs, {actual_wickets_taken} wickets)."
            f" Substituting {alt_bowler.name} (projected {exp_runs_alt:.1f}"
            " runs) would have reduced defensive win probability by"
            f" {abs(impact):.1f} percentage points."
        )
    else:
        verdict = "Neutral Impact: Comparable tactical options"
        explanation = (
            f"Both {first_ball.bowler.name} and {alt_bowler.name} projected"
            f" similar match equity (impact {impact:+.1f} percentage"
            " points)."
        )
