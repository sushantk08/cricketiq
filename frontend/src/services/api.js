...
export async function fetchPlayerIntelligence(playerId) {
  const res = await fetch(`${API_BASE}/players/${playerId}/intelligence`)
  if (!res.ok) throw new Error('Failed to fetch player intelligence')
  return res.json()
}
...
export async function fetchStrategyLab(strategyData) {
  const res = await fetch(`${API_BASE}/strategy-lab/analyze`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  if (!res.ok) {
    ...
    throw new Error(err.detail || 'Failed to analyze strategy lab')
  }
